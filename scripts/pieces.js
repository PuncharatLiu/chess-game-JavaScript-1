...
export const isOpponentPiece = (
  ...
  return (
    (overlapWhite.includes(pair) &&
      turn === "black" &&
      pair !== kingPosition) ||
    (overlapBlack.includes(pair) && turn === "white" && pair !== kingPosition)
  );
};

export const Is = {
  // to check what is click piece
  rook: function (getPieceId, fromPieceId) {
  ...
  blackPawn: function (getPieceId, fromPieceId) {
    return (
      (getPieceId >= 8 && getPieceId <= 15) ||
      (parseInt(fromPieceId) >= 8 && parseInt(fromPieceId) <= 15)
    );
  },
};
